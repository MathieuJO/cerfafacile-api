...
# Chemin vers le Cerfa vierge (à uploader dans le même dossier)
...
@app.route("/generer-cerfa", methods=["POST", "OPTIONS"])
def generer_cerfa():
    # CORS — autorise les appels depuis ton site Hostinger
    ...
    try:
        ...
        # Date et heure actuelles pour la déclaration
        now     = datetime.now()
        ...
        # Décomposer l'adresse vendeur (numéro + rue)
        vendeur_num_voie  = ""
        vendeur_type_voie = ""
        ...
        for p in [1, 2]:
            ...
            fields.update({
                ...
                f"{px}.num_KilométrageCompteur[0]":          str(km),
